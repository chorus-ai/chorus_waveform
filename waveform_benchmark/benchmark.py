...
MEM_METHOD = 3  # 1 is rusage, 2 is tracemalloc, 3 is psutils, 0 is for doing it outside of this function.
...
def _run_read_test(fmt, path, total_length, all_channels, block_length, block_count,
                   test_min_dur = 10, test_min_iter = 3, mem_method = 0):
    counters = []
    for i in repeat_test(test_min_dur, test_min_iter):
        r = random.Random(12345)
        with PerformanceCounter(mem_method = mem_method) as pc:
            for j in range(block_count):
                t0 = r.random() * (total_length - block_length)
                t1 = t0 + block_length
                fmt().read_waveforms(path, t0, t1, all_channels)    
                # mem = memory_usage((fmt().read_waveforms, (path, t0, t1, all_channels), {}), include_children = True, max_usage = True)
...
def run_benchmarks(input_record, format_class, pn_dir=None, format_list=None, waveform_list=None, test_list=None,
    ...
    with tempfile.TemporaryDirectory(prefix='wavetest-', dir='.') as tempdir:
        ...
        time1 = time.time()
        with PerformanceCounter(mem_method = MEM_METHOD) as pc_write:
            if (MEM_METHOD == 0):
        ...
        if (MEM_METHOD == 0):
            print('Maximum Memory Used: %.0f MiB' % mem_usage)
        else:
            print('Maximum Memory Used: %.0f MiB' % pc_write.max_memory)
        ...
        print(' #seek  #read      KiB      sec   walltime          Mem MB (curr/max)    [N]')
        ...
        for block_length, block_count in TEST_BLOCK_LENGTHS:
            ...
            counters = []
            if (MEM_METHOD == 0):
                mem_usage, counters = memory_usage((_run_read_test, (fmt, path, total_length, all_channels, block_length, block_count), {'test_min_dur': TEST_MIN_DURATION, 'test_min_iter': TEST_MIN_ITERATIONS, 'mem_method': MEM_METHOD}), include_children = True, max_usage = True, retval = True)
            # mem_usage = []
            else:
                counters = _run_read_test(fmt, path, total_length, all_channels, block_length, block_count, test_min_dur =TEST_MIN_DURATION, test_min_iter = TEST_MIN_ITERATIONS, mem_method = MEM_METHOD)
            walltime = time.time() - time1
            
            print('%6.0f %6.0f %8.0f %8.4f   %8.4f   %8.4f/%8.4f %6s read %d x %.0fs, all channels'
                  % (median_attr(counters, 'n_seek_calls'),
                     ...
                     median_attr(counters, 'curr_memory') if (MEM_METHOD != 0) else 0,
                     median_attr(counters, 'max_memory') if (MEM_METHOD != 0) else  mem_usage,
                     '[%d]' % len(counters),
                     block_count,
                     block_length))
        ...
        for block_length, block_count in TEST_BLOCK_LENGTHS:
            ...
            counters = []
            if (MEM_METHOD == 0):
                mem_usage, counters = memory_usage((_run_read_test_1channel, (fmt, path, total_length, all_channels, block_length, block_count), {'test_min_dur': TEST_MIN_DURATION, 'test_min_iter': TEST_MIN_ITERATIONS, 'mem_method': MEM_METHOD}), include_children = True, max_usage = True, retval = True)
            else:
                counters = _run_read_test_1channel(fmt, path, total_length, all_channels, block_length, block_count, test_min_dur =TEST_MIN_DURATION, test_min_iter = TEST_MIN_ITERATIONS, mem_method = MEM_METHOD )
            walltime = time.time() - time1
            
            print('%6.0f %6.0f %8.0f %8.4f   %8.4f   %8.4f/%8.4f %6s read %d x %.0fs, one channel'
                  % (median_attr(counters, 'n_seek_calls'),
                     median_attr(counters, 'n_read_calls'),
                     median_attr(counters, 'n_bytes_read') / 1024,
                     median_attr(counters, 'cpu_seconds'),
